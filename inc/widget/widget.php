...
function legal_advisory_widgets_init() {
	register_sidebar(
		array(
			...
			'after_widget'  => '</div>',
			'before_title'  => '<div class="subscr__title"><svg width="19" height="19">
 ...
 */
function legal_advisory_init_widgets() {
	register_widget('legal_advisory_About_Widget');
	register_widget('legal_advisory_Category_Widget');
	register_widget('legal_advisory_Subscribe_Widget');
	register_widget('legal_advisory_Customsearch_Widget');



}

add_action('widgets_init', 'legal_advisory_init_widgets');
...
class Aletheme_WidgetImageField 
{
	...
	function __construct($widget = null, $image_id = 0) {
		...
		if ($uri && in_array($file, array('widgets.php')) && is_admin()) {
			...
			wp_enqueue_script('widgetimagefield', get_template_directory() . '/assets/js/libs/widgets.js', array('jquery', 'jquery-ui-core', 'thickbox', 'media-upload'), false, true);
